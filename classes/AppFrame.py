...
from classes.IntInput import IntInput
from classes.LabelledSlider import LabelledSlider


class AppFrame(ctk.CTkScrollableFrame, TkinterDnD.DnDWrapper):
    settings = {}
    currentRow = 0
    settingsRowStart = 0
    activeSettings = []
    ...
    def addSettings(self):
        for setting in self.settings.keys():
            ...
            if data['type'] == 'text':
                ...
                inp = LabelledSlider(self, from_=0.01, to=5, value=data['value'], suffix='%', isPercentage=True)
                inp.grid(row=row, column=1)
                self.inputs[setting] = inp
